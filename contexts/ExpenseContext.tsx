...
export function ExpenseProvider({ children }: { children: ReactNode }) {
    ...
    const refreshGroups = async () => {
        ...
        try {
            console.log('🔄 [REFRESH GROUPS] Fetching groups...');
            const { data, error } = await supabase
                .from('group_members')
            ...
            console.log('✅ [REFRESH GROUPS] Refresh complete');
        } catch (error) {
            console.error('❌ [REFRESH GROUPS] Error:', error);
        }
    };

    // Fetch expenses when groupId changesync (address: string, username?: string) => {
